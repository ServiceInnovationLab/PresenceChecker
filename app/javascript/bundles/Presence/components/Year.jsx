...
export default class Year extends React.Component {
  ...
  render() {
    const { year, daysPresent, meetsAmountOfDaysInNZ, periodsAway } = this.props
      ...
      <Table className={`year-table is-text-center`} cellPadding="0" cellSpacing="0">
        <tr>
          ...
          <td
            ...
            <i className={`fas  ${meetsAmountOfDaysInNZ ? "fa-check" : "fa-times"}`} />
          </td>
        </tr>
        <tr>
          <td colSpan="4">
            <Table className="details-table">
              <tr className={`is-light ${isCollapsed ? "u-hide" : ""}`}>
                <td colSpan="2" className="secondary-header has-bottom-border has-border--light">
              ...
              {periodsAway.map(({ start, end, total }, index) => {
                return (
                  <tr
                    className={`is-light is-text-left date-range ${isCollapsed ? "u-hide" : ""}`}
